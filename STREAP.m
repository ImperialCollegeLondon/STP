...
load('obsAnnualRain.mat')%13*13 grids
domainSize = 13;
...
load('Files\gaugeIsEvent.mat','gaugeIsEvent');
...
for m=1:12
    ...
    [ QField(mmin(m,1):mmax(m,1),:,:) ] = quantileFieldGen( [domainSize*2 domainSize*2] , ARMA , 5 , ...
...
for d=1:365
    ...
    k(isinf(k))=1;
    simRain(1+288*(d-1):288*d,:,:)=simRain(1+288*(d-1):288*d,:,:).*repmat(reshape(k,[1 domainSize domainSize]),288,1,1);
end
