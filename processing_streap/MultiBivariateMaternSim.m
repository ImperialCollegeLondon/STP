function [ s1 , s2 ] = MultiBivariateMaternSim( N1 , N2 , A , N12 , RHO12 , N , K )
%BivariateMaternSim: Simulates a gaussian vector process with specific
...
for i = 1:N/2+1
    try
        ...
        temp = [sp11(i) sp12(i); sp12(i) sp22(i)];
        if i<N/2+1 && i~=1
            series{i,1} = mvnrnd([0 0],temp,K)'+1i*mvnrnd([0 0],temp,K)';
        else
            series{i,1} = mvnrnd([0 0],temp,K)';
        end
    catch
        ...
        while checkEig(1)<=1*10^-10 % SIGMA not a symmetric positive semi-definite matrix
            ...
            temp=temp+min(min([temp(1,1) temp(2,2)]))*.01*eye(2);
            checkEig=eig(temp+temp')/2;
        end
        if i<N/2+1 && i~=1
            series{i,1} = mvnrnd([0 0],temp,K)'+1i*mvnrnd([0 0],temp,K)';
