function [ Norm , Copula , MaternCov ] = biNormTransform( IsEvent , rIMF , rWAR , rTime , prdata)
%binormTransform A function that: (1) normalize the WAR and IMF; (2) Copula
%between WAR, IMF and event duration; (3) fits the VARFIMA parameters.
%   Inputs:
...
for m=1:12
    ...
    [Rho,nu] = copulafit('t',[u_q' v_q' w_q' z_q' q_q'],'Method','ApproximateML');
...
for m=1:12
    [ N1 , N2 , A , N12 , RHO12 ] = BivariateParsimoniousMaternFit( prdata.WAR(m).WAR , prdata.IMF(m).IMF , 100 );
    % TrivariateParsimoniousMaternFit( prdata.WAR(m).WAR , prdata.IMF(m).IMF , clddata.Wet(m).CAR , 100 );
    MaternCov(m).data = [ N1 , N2 , A , N12 , RHO12 ];
end
end