...
%   Zbas - watershed elevation [m]
%   Tdew - dew point temperature [°C]
%   So1 - Extraterrestrial Radiation VIS band [0.29 um - 0.70 um ] [W/m^2]
%   So2 - Extraterrestrial Radiation NIR band [0.70 um - 4.0 um ] [W/m^2]
%   beta_A - Angstrom turbidity parameters, 0.05 [0-1] 
%   alpha_A - Angstrom turbidity parameters, 1.3 [0.5-2.5] 
%   omega_A1 - aerosol single-scattering albedo band 1, 0.92 [0.74-0.94]
%   omega_A2 - aerosol single-scattering albedo band 2, 0.84 [0.74-0.94]
%	uo - ozone amount in vertical column, 0.35 [0.22-0.35] [cm]
%	un - total nitrogen dioxide amount, 0.0002 [0.0001-0.046] [cm]
%	rho_g  - spatial average regional albedo, 0.15 [0.05-0.3] [-]
%   corrAOD - elevation correction for the aerosol optical depth (after
%   Ingold, 2001)
%   zAOD - elevation of the aerosol optical depth station [m]
%	Outputs:
%	Eb1 - clear sky beam irradiance VIS band [0.29 um - 0.70 um ] [W/m^2]
%	Eb2 - clear sky beam irradiance NIR band [0.70 um - 4.0 um ] [W/m^2]
%	Ed1 - clear sky total diffuse flux at the ground, VIS band [W/m^2]
%	Ed2 - clear sky total diffuse flux at the ground, NIR band [W/m^2]
%   Edp1
%   Edp2
%   rho_s1
%   rho_s2
...
if h_S>0.0 % if there is sunshine
    ...
    p=1013.25.*exp(-Zbas./8434.5); %%% [mbar] Pressure after correction for differences in pressure between basin and sea level
...
%     Eb1 = So1*prod(T1);
%     Eb2 = So2*prod(T2);
    Eb1=So1.*TO1.*TR1.*TG1.*TW1.*TA1.*TN1;
    Eb2=So2.*TO2.*TR2.*TG2.*TW2.*TA2.*TN2;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
    ...
    rho_s1=(0.13363+0.00077358.*alpha1+beta1.*(0.37567+0.22946.*alpha1)./(1-0.10832.*alpha1))./(1+beta1.*(0.84057+0.68683.*alpha1)./(1-0.08158.*alpha1));
    rho_s2=(0.010191+0.00085547.*alpha2+beta2.*(0.14618+0.062758.*alpha2)./(1-0.19402.*alpha2))./(1+beta2.*(0.58101+0.17426.*alpha2)./(1-0.17586.*alpha2));
...
%     Edp2= TO2.*TG2.*TN2p.*TW2p.*(BR2.*(1-TR2).*(TA2.^0.25) +Ba.*F2.*TR2.*(1-TAs2.^0.25)).*So2.*sin(h_S);
    Edp1= TO1.*TG1.*TN1p.*TW1p.*(BR1.*(1-TR1).*(sqrt(sqrt(TA1))) +Ba.*F1.*TR1.*(1-sqrt(sqrt(TAs1)))).*So1.*sin(h_S); % Faster
    Edp2= TO2.*TG2.*TN2p.*TW2p.*(BR2.*(1-TR2).*(sqrt(sqrt(TA2))) +Ba.*F2.*TR2.*(1-sqrt(sqrt(TAs2)))).*So2.*sin(h_S);
    ...
    Ed1 = zeros(size(Zbas));
    Ed2 = zeros(size(Zbas));
    Edp1=zeros(size(Zbas));
    Edp2=zeros(size(Zbas));
    rho_s1=nan(size(Zbas));
    rho_s2=nan(size(Zbas));
    Mb=zeros(size(Zbas)); 
    Mg=zeros(size(Zbas)); 
end
end