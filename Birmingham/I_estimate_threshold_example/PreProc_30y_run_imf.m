...
I_s = 8;% 30; % Number of years to simulate
dx = 1;
dt = 5;
domainSize = 110;
ARMA22 = ARMA(8);
%% Storm arrival process
[ dryPool , wetPool ] = drywetPool( I_s , Precipitation.data );
for I=1:I_s
    tic
    ...
    rField=zeros(105120,domainSize,domainSize);
    for m=1:12
        ...
        for i=1:size(tsMatrix,1)
            ...
            if tsMatrix(i,1)==1 && m==M
                [ rField(tsMatrix(i,3):tsMatrix(i,4),:,:) ] = invLN2( rField(tsMatrix(i,3):tsMatrix(i,4),:,:) , ...
