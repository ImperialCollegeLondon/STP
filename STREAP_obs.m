...
%% PreProcessing
%% Load data
filePath = 'Birmingham_obs/';%'Engelberger/';%
dx = 1;%2
dt = 5;
domainSize = 110;%13
...
% calibrate()
generate(dx, dt, domainSize, WAR_threshold, isEventdt, filePath);

function generate(dx, dt, domainSize, WAR_threshold, isEventdt, filePath)
%% Processing
...
load([filePath, 'ARMA.mat'])

for I = 41:12*4; % Number of realizations to simulate (the processing stage can be looped)
...
ARMA22 = ARMA(1);%ARMA(8);
arma_decayA = -0.0067*minutes(dt)/minutes(5);
% ARMA22 = ARMA;
simRain = zeros(days(365)/minutes(dt),domainSize,domainSize);
QField = [];
...
for m=1:12
    ...
    % ARMA22=1-(1-ARMA22.ar)*minutes(dt)/minutes(5);
    ...
    [ QField(mmin(m,1):mmax(m,1),:,:) ] = quantileFieldGen_ARMA( [domainSize*2 domainSize*2] , arma_decayA , 5 , ...
        Precipitation.data(m).SpatialAlpha , [mmin(m,1) mmax(m,1)] , simU , simV , dx , dt );
...
[Time,XX,YY] = getDataAttributes(days(365)/minutes(dt),dt);
simDATA = RainfallDataClass(simRain,-1,32,'mm/h',...
    Time,XX,YY);
save(['H:\DATA_STREAP\',sprintf('Output_Pr_sim_%02d.mat',I)],'simDATA','simIMF','simWAR','simU','simV','-v7.3');
...
A = load([filePath,'MeanArealStats.mat']); % WAR [-], IMF [mm h^-1] data
% U500, V500: [m s^-1]
% MeanArealStats = struct;
% MeanArealStats.WAR = cat(1,[],A.MeanArealStats.WAR);
% MeanArealStats.IMF = cat(1,[],A.MeanArealStats.IMF);
% MeanArealStats.U500 = cat(1,[],A.MeanArealStats.U500);
% MeanArealStats.V500 = cat(1,[],A.MeanArealStats.V500);
% MeanArealStats.Time = cat(1,[],A.MeanArealStats.Time);
% clear A
...
% only look at ensemble 1 %%%%
MeanArealStats = A.MeanArealStats(1);
MeanArealStats.Time = datenum(MeanArealStats.Time);
CV = CV{1};
NHRO = NHRO{1};
mkdir('Files\')
mkdir('Figs\tif')
...
% not good:[ radarIsEvent ] = findRadarEvents( MeanArealStats.WAR , WAR_threshold , 60 );
[ radarIsEvent ] = findRainfallEvents( MeanArealStats.WAR, WAR_threshold, 1, 60);
...
inputInfo = getInputInfo(region,'radar');%'cpm-historical');
